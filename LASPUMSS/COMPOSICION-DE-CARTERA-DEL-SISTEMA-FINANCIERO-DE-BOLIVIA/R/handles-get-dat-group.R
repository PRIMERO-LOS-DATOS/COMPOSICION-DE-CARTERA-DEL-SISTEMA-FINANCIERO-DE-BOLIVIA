...
handles$getDatEstFinByGroupAsfi <- function(dat=NULL, by='ENTIDAD', desagregados = TRUE, yearInit = 2014, accounts=NULL, rootDatXlsx = NULL, byClass = NULL, byEnts = NULL, allAccounts=FALSE) {
    ...
    if (is.null(dat) && !desagregados) { dat <- handles$getDatEEFF(gestionIncial= yearInit) }
    
    if (is.null(dat) && desagregados) { 
        dat <- handles$getDatEstFinDesagregadosAsfi(yearInit = yearInit, 
                                                    accounts = accounts,
